...
function getFizzBuzz(num) {
  ...
  if (!(num % 3) && !(num % 5)) result = 'FizzBuzz';
  return result;
}
...
function doRectanglesOverlap(rect1, rect2) {
  ...
  return !(rect1X1 < rect2.left
    || rect1.left > rect2X1 || rect1.top > rect2Y1 || rect1Y1 < rect2.top);
}
...
function isInsideCircle(circle, point) {
  const d = ((point.x - circle.center.x) ** 2 + (point.y - circle.center.y) ** 2) ** 0.5;
...
function isCreditCardNumber(ccn) {
  const arr = ccn.toString().split('').map((x) => parseInt(x, 10));
...
function getDigitalRoot(num) {
  ...
  return getDigitalRoot(String(num).split('').reduce((acc, cur) => acc + Number(cur), 0));
}
// function getDigitalRoot(num) {
//   let newNum = num;
//   while (String(newNum).length !== 1) {
//     newNum = String(newNum).split('').reduce((acc, cur) => acc + Number(cur), 0);
//   }
//   return newNum;
// }
...
function isBracketsBalanced(str) {
  const newStr = str.replace('[]', '')
    ...
    .replace('{}', '')
    .replace('<>', '');
  if (newStr.length === str.length) return !newStr.length;
...
function getMatrixProduct(m1, m2) {
  const multiplication = new Array(m1.length);
  for (let x = 0; x < multiplication.length; x += 1) {
    multiplication[x] = new Array(m2[0].length).fill(0);
  }

  for (let x = 0; x < multiplication.length; x += 1) {
    for (let y = 0; y < multiplication[x].length; y += 1) {
      for (let z = 0; z < m1[0].length; z += 1) {
        multiplication[x][y] += m1[x][z] * m2[z][y];
      }
    }
  }
...
// function getMatrixProduct(m1, m2) {
//   const multiplication = Array.from({ length: m1.length }, () =>
// Array.from({ length: m2[0].length }, (el) => (el = 0)));
